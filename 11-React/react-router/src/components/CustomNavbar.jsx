    ...
    return (
        <nav className={`bg-white pb-5 md:text-sm ${state ? "shadow-lg rounded-xl border mx-2 mt-2 md:shadow-none md:border-none md:mx-2 md:mt-0" : ""}`}>
            <div className="gap-x-14 items-center max-w-screen-xl mx-auto px-4 md:flex md:px-8">
                ...
                <div className={`flex-1 items-center mt-8 md:mt-0 md:flex justify-end ${state ? 'block' : 'hidden'} `}>
                    <ul className="justify-center items-center space-y-6 md:flex md:space-x-6 md:space-y-0">
                        {
                            navigation.map((item, idx) => {
                                return ( // text-blue-500
                                    <li key={idx} className="text-gray-700 hover:text-gray-900">
                                        <NavLink to={item.path} className={({ isActive }) => `block ${isActive ? "text-blue-500" : ""}`}>
                                            {item.title}
                                        </NavLink>
                                    </li>
                                )
